# ----------------------------------------------------------------------------------------------------
# 틀린 코드: make_dp를 두 번 돌게 했지만, RGB 색상은 3개이므로 3번 돌아야 한다. 3번 돌릴 때마다 처음 방문하는 색상만 0으로 두고 나머지는 inf로 둬야한다.
...
# ----------------------------------------------------------------------------------------------------
# 수정 코드: 3번 돌았다. make_dp에서, dp에는 i에 해당하는 인덱스 값에만 RGBs의 값을 대입한다. 그리고 아래로 쭉 내려가다 보면 처음에 i번째 집을 방문한 경우의 값들만 나온다. 마지막 집은 첫 번째 집과 겹치면 안 되므로 i값을 제외한 나머지 dp값들을 비교하여 최소값을 도출한다. 이것을 3번 반복한다.
import sys; input=sys.stdin.readline
